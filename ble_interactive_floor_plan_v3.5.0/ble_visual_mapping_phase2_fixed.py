"""
BLE Enhanced Terminal Visual Mapping System - PHASE 2: INTERACTIVE FLOOR PLAN v3.5.0
Features: Multi-Room/Floor Support, Drag & Drop, Undo/Delete, ESP32 Node Positioning, Real-Time Device Tracking
...
<body>
            ...
            <div class="floorplan-container">
                ...
                <div class="floorplan-sidebar">
                    ...
                    <div style="margin-bottom: 1rem;">
                        <h4>🏢 Floor/Room Management</h4>
                        
                        <!-- Current Floor Display -->
                        <div style="margin-bottom: 0.5rem; padding: 0.5rem; background: #e8f4fd; border-radius: 4px; border-left: 4px solid #667eea;">
                            <div style="font-weight: bold; color: #667eea;">Current Floor:</div>
                            <div id="currentFloorDisplay" style="font-size: 0.9rem;">Ground Floor</div>
                        </div>
                        
                        <!-- Floor Navigation -->
                        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 0.3rem; margin-bottom: 0.5rem;">
                            <button class="btn" onclick="createNewFloor()" title="Create new floor/room">➕ New</button>
                            <button class="btn" onclick="switchFloor()" title="Switch between floors">🔄 Switch</button>
                            <button class="btn" onclick="manageFloors()" title="Manage all floors">⚙️ Manage</button>
                        </div>
                        
                        <!-- Quick Floor List -->
                        <div id="floorQuickList" style="max-height: 100px; overflow-y: auto; font-size: 0.8rem;">
                            <!-- Floor list will be populated here -->
                        </div>
                    </div>
                    ...
                    <div style="margin-bottom: 1rem;">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem;">
                            <button class="btn" onclick="undoLastAction()" id="undoBtn" title="Undo last action (Ctrl+Z)">↶ Undo</button>
                            <button class="btn" onclick="deleteSelectedObject()" id="deleteBtn" title="Delete selected object (Delete key)" style="background: #e74c3c;">🗑️ Delete</button>
                        </div>
                        <button class="btn" onclick="clearAllObjects()" style="background: #c0392b; margin-top: 0.5rem; width: 100%;">🗑️ Clear All</button>
                    ...
                    <!-- ESP32 Node Placement -->
                    <div style="margin-bottom: 1rem;">
                        <h4>📡 ESP32 Node Placement</h4>
                        <div style="margin-bottom: 0.5rem;">
                            <select id="esp32NodeSelect" style="width: 100%; padding: 0.3rem; border: 1px solid #ddd; border-radius: 4px;">
                                <option value="">Select ESP32 node to place...</option>
                            </select>
                        </div>
                        <button class="btn" onclick="placeSelectedESP32Node()" style="width: 100%; background: #667eea; color: white;">📍 Place Node on Floor Plan</button>
                        
                        <!-- Current Floor Nodes -->
                        <div style="margin-top: 0.5rem; font-size: 0.8rem;">
                            <div style="font-weight: bold; margin-bottom: 0.3rem;">Nodes on Current Floor:</div>
                            <div id="currentFloorNodes" style="max-height: 80px; overflow-y: auto;">
                    ...
                    <div style="margin-bottom: 1rem;">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem; margin-bottom: 0.5rem;">
                            <button class="btn" onclick="undoLastAction()" title="Undo last action (Ctrl+Z)">↶ Undo</button>
                            <button class="btn" onclick="deleteSelectedObject()" title="Delete selected object (Delete key)">🗑️ Delete</button>
                        </div>
                        <button class="btn" onclick="clearAllObjects()" style="width: 100%; background-color: #dc3545; color: white;" title="Clear all objects">🗑️ Clear All</button>
                        <div id="objectInfo" style="font-size: 0.8rem; color: #666; margin-top: 0.5rem;">
                            Objects: <span id="objectCount">0</span> | Selected: <span id="selectedInfo">None</span>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        // Multi-floor/room variables
        let currentFloorId = 1;
        let currentFloorName = 'Ground Floor';
        let allFloors = [];
        let floorPlanData = {}; // Store floor plan data for each floor
        
        // Undo/Delete functionality variables
        let undoHistory = [];
        let maxUndoSteps = 20;
        ...
        function initFloorPlanCanvas() {
            ...
            floorPlanCanvas.addEventListener('contextmenu', onFloorPlanRightClick);
            
            // Add keyboard event listeners for shortcuts
            document.addEventListener('keydown', onFloorPlanKeyDown);
        ...
        function onFloorPlanMouseDown(e) {
            ...
            // Drawing mode - save state before drawing
            if (currentTool !== 'select') {
                saveStateToHistory();
                startX = mouseX;
                startY = mouseY;
        ...
        function drawFloorPlan() {
            ...
            floorPlanObjects.forEach(obj => {
                if (obj.type === 'esp32_node') {
                    drawFloorPlanObjectEnhanced(obj);
                } else {
        ...
        function saveStateToHistory() {
            // Deep copy the current floor plan objects
            const currentState = JSON.parse(JSON.stringify(floorPlanObjects));
            ...
            updateObjectInfo();
            console.log('💾 Saved state to history. History length:', undoHistory.length);
        }

        function undoLastAction() {
            if (undoHistory.length > 0) {
                floorPlanObjects = undoHistory.pop();
                selectedObject = null;
                drawFloorPlan();
                updateObjectInfo();
                console.log('↶ Undid last action. History length:', undoHistory.length);
            } else {
        ...
        function deleteSelectedObject() {
            if (selectedObject) {
                ...
                const index = floorPlanObjects.findIndex(obj => obj === selectedObject);
                if (index !== -1) {
                    floorPlanObjects.splice(index, 1);
                    console.log('🗑️ Deleted object:', selectedObject.type);
                    selectedObject = null;
                    drawFloorPlan();
                    updateObjectInfo();
                }
        ...
        function clearAllObjects() {
            if (floorPlanObjects.length > 0) {
                if (confirm('Are you sure you want to clear all objects? This cannot be undone.')) {
                    ...
                    drawFloorPlan();
                    updateObjectInfo();
                    console.log('🗑️ Cleared all objects');
                }
            }
        }

        function updateObjectInfo() {
            document.getElementById('objectCount').textContent = floorPlanObjects.length;
            document.getElementById('selectedInfo').textContent = selectedObject ? 
                `${selectedObject.type} (${Math.round(selectedObject.x)}, ${Math.round(selectedObject.y)})` : 'None';
        }
        ...
        function selectFurniture(type) {
            saveStateToHistory(); // Save state before adding furniture
        ...
        // ESP32 Node Placement Functions
        function loadESP32NodesForPlacement() {
            fetch('/api/esp32-nodes')
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        updateESP32NodeSelect(data.esp32_nodes);
                        updateCurrentFloorNodes(data.esp32_nodes);
                    }
                })
                .catch(error => {
        ...
        function updateESP32NodeSelect(nodes) {
            const select = document.getElementById('esp32NodeSelect');
            if (!select) return;
            
            // Clear existing options except the first one
            select.innerHTML = '<option value="">Select ESP32 node to place...</option>';
            
            // Add nodes that are not assigned to the current floor
            const currentFloorNodes = nodes.filter(node => node.floor_plan_id === currentFloorId);
            const availableNodes = nodes.filter(node => node.floor_plan_id !== currentFloorId);
            
            availableNodes.forEach(node => {
                const option = document.createElement('option');
                option.value = node.id;
                option.textContent = `${node.name} (${node.node_id}) - ${node.room_name}`;
            ...
            if (availableNodes.length === 0) {
                const option = document.createElement('option');
                option.value = '';
                option.textContent = 'All nodes are assigned to floors';
                option.disabled = true;
                select.appendChild(option);
            }
        }

        function updateCurrentFloorNodes(nodes) {
            const container = document.getElementById('currentFloorNodes');
            if (!container) return;
            
            const currentFloorNodes = nodes.filter(node => node.floor_plan_id === currentFloorId);
            
            if (currentFloorNodes.length === 0) {
                container.innerHTML = '<div style="color: #666; font-style: italic;">No nodes on this floor</div>';
                return;
            }
            
            container.innerHTML = currentFloorNodes.map(node => `
                <div style="padding: 0.3rem; margin: 0.2rem 0; background: #f8f9fa; border-radius: 3px; border-left: 3px solid #667eea;">
                    <div style="font-weight: bold; color: #333; font-size: 0.8rem;">📡 ${node.name}</div>
                    <div style="color: #666; font-size: 0.7rem;">${node.node_id} • ${node.room_name}</div>
                    <div style="color: #666; font-size: 0.7rem;">Range: ${node.max_distance}m</div>
                </div>
            `).join('');
        }

        function placeSelectedESP32Node() {
            const select = document.getElementById('esp32NodeSelect');
            const nodeId = select.value;
            
            if (!nodeId) {
                alert('Please select an ESP32 node to place');
                return;
            }
            
            // Get node details
            const selectedOption = select.options[select.selectedIndex];
            const nodeName = selectedOption.textContent.split(' (')[0];
            
            // Save state before adding node
            saveStateToHistory();
            
            // Add ESP32 node to floor plan
            const nodeObj = {
                type: 'esp32_node',
            ...
            floorPlanObjects.push(nodeObj);
            
            // Update node's floor assignment in database
            fetch(`/api/esp32-nodes/${nodeId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ 
                    floor_plan_id: currentFloorId,
                    x_position: nodeObj.x,
                    y_position: nodeObj.y
                })
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    console.log('✅ Assigned ESP32 node to floor:', nodeName);
                    loadESP32NodesForPlacement(); // Refresh the lists
                    drawFloorPlan();
                    updateObjectInfo();
                    
                    // Automatically switch to select tool for immediate positioning
                    selectTool('select');
                    selectedObject = nodeObj;
                    updateObjectInfo();
                } else {
                    alert('Error assigning node to floor: ' + data.error);
                    // Remove the node from floor plan if database update failed
                    const index = floorPlanObjects.findIndex(obj => obj === nodeObj);
                    if (index !== -1) {
                        floorPlanObjects.splice(index, 1);
                        drawFloorPlan();
                        updateObjectInfo();
            ...
            .catch(error => {
                alert('Error assigning node to floor: ' + error.message);
                // Remove the node from floor plan if request failed
                const index = floorPlanObjects.findIndex(obj => obj === nodeObj);
                if (index !== -1) {
                    floorPlanObjects.splice(index, 1);
                    drawFloorPlan();
        ...
        // Enhanced drawFloorPlanObject to handle ESP32 nodes
        function drawFloorPlanObjectEnhanced(obj) {
            ...
            // Set styles
            if (obj.style) {
                floorPlanCtx.strokeStyle = obj.style.strokeStyle || '#667eea';
                floorPlanCtx.lineWidth = obj.style.lineWidth || 2;
                floorPlanCtx.fillStyle = obj.style.fillStyle || 'rgba(102, 126, 234, 0.1)';
            }
            ...
            switch(obj.type) {
                ...
                case 'esp32_node':
                    // Draw ESP32 node with special styling
                    floorPlanCtx.fillStyle = '#667eea';
                    floorPlanCtx.strokeStyle = '#4c63d2';
                    floorPlanCtx.lineWidth = 2;
                    
                    // Draw circle background
                    floorPlanCtx.beginPath();
                    floorPlanCtx.arc(obj.x + obj.width/2, obj.y + obj.height/2, obj.width/2, 0, 2 * Math.PI);
                    floorPlanCtx.fill();
                    floorPlanCtx.stroke();
                    
                    // Draw emoji
                    floorPlanCtx.font = `${obj.width * 0.6}px Arial`;
                    floorPlanCtx.fillStyle = 'white';
                    floorPlanCtx.textAlign = 'center';
                    floorPlanCtx.fillText(obj.emoji, obj.x + obj.width/2, obj.y + obj.height/2 + obj.width * 0.2);
                    
                    // Draw node name
                    floorPlanCtx.font = '10px Arial';
                    floorPlanCtx.fillStyle = '#333';
                    floorPlanCtx.fillText(obj.nodeName, obj.x + obj.width/2, obj.y + obj.height + 12);
                    break;
            }
        ...
        // Keyboard shortcuts and context menu functions
        function onFloorPlanKeyDown(e) {
            // Only handle keyboard shortcuts when Interactive Floor Plan tab is active
            const activeTab = document.querySelector('.tab-content:not([style*="display: none"])');
            if (!activeTab || activeTab.id !== 'floorplan-tab') return;
        ...
        function onFloorPlanRightClick(e) {
            e.preventDefault();
            ...
            if (clickedObject) {
                ...
                console.log('🖱️ Right-clicked on object:', clickedObject.type);
            } else {
        ...
        function showContextMenu(x, y, object) {
            ...
            if (object) {
                menuItems = [
                    { text: '🗑️ Delete', action: () => deleteSelectedObject() },
                    { text: '📋 Copy', action: () => copyObject(object) },
                    { text: '📍 Bring to Front', action: () => bringToFront(object) },
                    { text: '📍 Send to Back', action: () => sendToBack(object) }
                ];
            } else {
                menuItems = [
                    { text: '📋 Paste', action: () => pasteObject(x - rect.left, y - rect.top) },
                    { text: '↶ Undo', action: () => undoLastAction() },
                    { text: '🗑️ Clear All', action: () => clearAllObjects() }
                ];
            }
        ...
        function copyObject(object) {
            ...
            console.log('📋 Copied object:', object.type);
        }
        ...
        function pasteObject(x, y) {
            if (copiedObject) {
                ...
                floorPlanObjects.push(newObject);
                selectedObject = newObject;
                drawFloorPlan();
                updateObjectInfo();
                console.log('📋 Pasted object at:', x, y);
            }
        }

        function bringToFront(object) {
            const index = floorPlanObjects.findIndex(obj => obj === object);
            if (index !== -1) {
                saveStateToHistory();
                floorPlanObjects.splice(index, 1);
                floorPlanObjects.push(object);
                drawFloorPlan();
                console.log('📍 Brought object to front:', object.type);
            }
        }
        ...
        function sendToBack(object) {
            ...
            if (index !== -1) {
                saveStateToHistory();
                floorPlanObjects.splice(index, 1);
                floorPlanObjects.unshift(object);
                drawFloorPlan();
                console.log('📍 Sent object to back:', object.type);
            }
        }
        ...
        // Multi-floor/room management functions
        function loadAllFloors() {
            fetch('/api/floor-plans')
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        allFloors = data.floor_plans;
                        updateFloorQuickList();
                        updateCurrentFloorDisplay();
                        console.log('📋 Loaded floors:', allFloors.length);
                    }
                })
                .catch(error => {
        ...
        function updateFloorQuickList() {
            const container = document.getElementById('floorQuickList');
            if (!container) return;
            
            if (allFloors.length === 0) {
                container.innerHTML = '<div style="color: #666; font-style: italic;">No floors created yet</div>';
                return;
            }
            
            container.innerHTML = allFloors.map(floor => `
                <div class="floor-item" style="padding: 0.3rem; margin: 0.2rem 0; background: ${floor.id === currentFloorId ? '#e8f4fd' : '#f8f9fa'}; border-radius: 3px; cursor: pointer; border-left: 3px solid ${floor.id === currentFloorId ? '#667eea' : 'transparent'};" onclick="switchToFloor(${floor.id})">
                    <div style="font-weight: ${floor.id === currentFloorId ? 'bold' : 'normal'}; color: ${floor.id === currentFloorId ? '#667eea' : '#333'};">
                        ${floor.name}
                    </div>
                    <div style="font-size: 0.7rem; color: #666;">
                        ${floor.data ? JSON.parse(floor.data).length || 0 : 0} objects
                    </div>
                </div>
            `).join('');
        }
        ...
        function updateCurrentFloorDisplay() {
            const displayElement = document.getElementById('currentFloorDisplay');
            if (displayElement) {
                displayElement.textContent = currentFloorName;
            }
        }

        function createNewFloor() {
            const floorName = prompt('Enter name for new floor/room:', `Floor ${allFloors.length + 1}`);
            ...
            .then(data => {
                if (data.success) {
                    console.log('✅ Created new floor:', floorName);
                    loadAllFloors();
                    
                    // Ask if user wants to switch to the new floor
                    if (confirm(`Floor "${floorName}" created successfully! Switch to it now?`)) {
                        switchToFloor(data.floor_id);
                    }
                } else {
                    alert('Error creating floor: ' + data.error);
                }
            })
            .catch(error => {
                alert('Error creating floor: ' + error.message);
            });
        }

        function switchFloor() {
            if (allFloors.length === 0) {
                alert('No floors available. Create a new floor first.');
                return;
            }
        ...
        function showFloorSelectionDialog() {
            // Remove existing dialog
            const existingDialog = document.getElementById('floorSelectionDialog');
            if (existingDialog) {
            ...
            dialog.id = 'floorSelectionDialog';
            ...
            dialog.innerHTML = `
                <div style="padding: 1rem; border-bottom: 1px solid #eee;">
                    <h3 style="margin: 0; color: #333;">🔄 Switch Floor/Room</h3>
                </div>
                <div style="padding: 1rem;">
                    ${allFloors.map(floor => `
                        <div class="floor-selection-item" style="padding: 0.8rem; margin: 0.5rem 0; background: ${floor.id === currentFloorId ? '#e8f4fd' : '#f8f9fa'}; border-radius: 4px; cursor: pointer; border: 2px solid ${floor.id === currentFloorId ? '#667eea' : 'transparent'};" onclick="selectFloorFromDialog(${floor.id})">
                            <div style="font-weight: bold; color: ${floor.id === currentFloorId ? '#667eea' : '#333'};">
                                ${floor.name} ${floor.id === currentFloorId ? '(Current)' : ''}
                            </div>
                            <div style="font-size: 0.8rem; color: #666; margin-top: 0.2rem;">
                                Created: ${new Date(floor.created_at).toLocaleDateString()}
                            </div>
                            <div style="font-size: 0.8rem; color: #666;">
                                Objects: ${floor.data ? JSON.parse(floor.data).length || 0 : 0}
                            </div>
                        </div>
                    `).join('')}
                </div>
                <div style="padding: 1rem; border-top: 1px solid #eee; text-align: right;">
                    <button onclick="closeFloorSelectionDialog()" style="padding: 0.5rem 1rem; background: #6c757d; color: white; border: none; border-radius: 4px; cursor: pointer;">Cancel</button>
                </div>
            `;
            ...
            backdrop.id = 'floorSelectionBackdrop';
            backdrop.style.cssText = `
                ...
                z-index: 999;
            `;
            backdrop.onclick = closeFloorSelectionDialog;
            document.body.appendChild(backdrop);
        }

        function selectFloorFromDialog(floorId) {
            closeFloorSelectionDialog();
            switchToFloor(floorId);
        }

        function closeFloorSelectionDialog() {
            const dialog = document.getElementById('floorSelectionDialog');
            const backdrop = document.getElementById('floorSelectionBackdrop');
            if (dialog) dialog.remove();
            if (backdrop) backdrop.remove();
        }

        function switchToFloor(floorId) {
            // Save current floor data before switching
            ...
            // Update current floor info
            currentFloorId = floorId;
            currentFloorName = floor.name;
            
            // Load floor data
            try {
                floorPlanObjects = floor.data ? JSON.parse(floor.data) : [];
                scale = floor.scale_factor || 50;
            ...
            console.log('🔄 Switched to floor:', currentFloorName);
        }

        function saveCurrentFloorData() {
            if (currentFloorId && floorPlanObjects.length > 0) {
                const floorData = {
                    data: JSON.stringify(floorPlanObjects),
                    scale_factor: scale
                };
                
                fetch(`/api/floor-plans/${currentFloorId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                ...
                .then(data => {
                    if (data.success) {
                        console.log('💾 Saved floor data for:', currentFloorName);
                        // Update the floor in allFloors array
                        const floorIndex = allFloors.findIndex(f => f.id === currentFloorId);
                        if (floorIndex !== -1) {
                            allFloors[floorIndex].data = JSON.stringify(floorPlanObjects);
                            allFloors[floorIndex].scale_factor = scale;
                        }
                    }
                })
                .catch(error => {
        ...
        function manageFloors() {
            showFloorManagementDialog();
        }

        function showFloorManagementDialog() {
            // Remove existing dialog
            const existingDialog = document.getElementById('floorManagementDialog');
            if (existingDialog) {
                existingDialog.remove();
            }
            ...
            // Create dialog
            const dialog = document.createElement('div');
            dialog.id = 'floorManagementDialog';
            dialog.style.cssText = `
                position: fixed;
                top: 50%;
                left: 50%;
            ...
            dialog.innerHTML = `
                <div style="padding: 1rem; border-bottom: 1px solid #eee;">
                    <h3 style="margin: 0; color: #333;">⚙️ Manage Floors/Rooms</h3>
                </div>
                <div style="padding: 1rem;">
                    ${allFloors.map(floor => `
                        <div class="floor-management-item" style="padding: 1rem; margin: 0.5rem 0; background: #f8f9fa; border-radius: 4px; border: 1px solid #dee2e6;">
                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                <div>
                                    ...
                                    <div style="font-size: 0.8rem; color: #666; margin-top: 0.2rem;">
                                        Created: ${new Date(floor.created_at).toLocaleDateString()} • Objects: ${floor.data ? JSON.parse(floor.data).length || 0 : 0}
                                    </div>
                                </div>
                                <div style="display: flex; gap: 0.5rem;">
                                    <button onclick="editFloorName(${floor.id}, '${floor.name}')" style="padding: 0.3rem 0.6rem; background: #667eea; color: white; border: none; border-radius: 3px; cursor: pointer; font-size: 0.8rem;">✏️ Edit</button>
                                    <button onclick="duplicateFloor(${floor.id})" style="padding: 0.3rem 0.6rem; background: #28a745; color: white; border: none; border-radius: 3px; cursor: pointer; font-size: 0.8rem;">📋 Copy</button>
                                    <button onclick="deleteFloor(${floor.id})" style="padding: 0.3rem 0.6rem; background: #dc3545; color: white; border: none; border-radius: 3px; cursor: pointer; font-size: 0.8rem;">🗑️ Delete</button>
                                </div>
                            </div>
                        </div>
                    `).join('')}
                ...
                <div style="padding: 1rem; border-top: 1px solid #eee; text-align: right;">
                    <button onclick="closeFloorManagementDialog()" style="padding: 0.5rem 1rem; background: #6c757d; color: white; border: none; border-radius: 4px; cursor: pointer;">Close</button>
                </div>
            `;
            ...
            // Add backdrop
            const backdrop = document.createElement('div');
            backdrop.id = 'floorManagementBackdrop';
            backdrop.style.cssText = `
                position: fixed;
                top: 0;
                left: 0;
                ...
                z-index: 999;
            `;
            backdrop.onclick = closeFloorManagementDialog;
            document.body.appendChild(backdrop);
        }

        function closeFloorManagementDialog() {
            const dialog = document.getElementById('floorManagementDialog');
            const backdrop = document.getElementById('floorManagementBackdrop');
            if (dialog) dialog.remove();
            if (backdrop) backdrop.remove();
        }

        function editFloorName(floorId, currentName) {
            const newName = prompt('Enter new name for floor/room:', currentName);
            ...
            .then(data => {
                if (data.success) {
                    console.log('✅ Renamed floor:', newName);
                    loadAllFloors();
                    closeFloorManagementDialog();
                    
                    // Update current floor name if it's the one being edited
                    if (floorId === currentFloorId) {
                        currentFloorName = newName.trim();
                        updateCurrentFloorDisplay();
                    }
                } else {
                    alert('Error renaming floor: ' + data.error);
                }
            })
            .catch(error => {
                alert('Error renaming floor: ' + error.message);
            });
        }
        ...
        function duplicateFloor(floorId) {
            ...
            const newName = prompt('Enter name for duplicated floor/room:', floor.name + ' (Copy)');
            ...
            fetch('/api/floor-plans', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(duplicatedFloor)
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    console.log('✅ Duplicated floor:', newName);
                    loadAllFloors();
                    closeFloorManagementDialog();
                } else {
                    alert('Error duplicating floor: ' + data.error);
                }
            })
            .catch(error => {
                alert('Error duplicating floor: ' + error.message);
            });
        }
        ...
        function deleteFloor(floorId) {
            ...
            if (!confirm(`Are you sure you want to delete "${floor.name}"? This cannot be undone.`)) return;
            
            if (floorId === currentFloorId && allFloors.length === 1) {
                alert('Cannot delete the only remaining floor. Create another floor first.');
                return;
            }
            ...
            .then(data => {
                if (data.success) {
                    console.log('🗑️ Deleted floor:', floor.name);
                    
                    // If deleting current floor, switch to first available floor
                    if (floorId === currentFloorId) {
                        const remainingFloors = allFloors.filter(f => f.id !== floorId);
                        if (remainingFloors.length > 0) {
                            switchToFloor(remainingFloors[0].id);
                        }
                    }
                    ...
                    alert('Error deleting floor: ' + data.error);
                }
            })
            .catch(error => {
        ...
        function switchTab(tabName) {
            ...
            if (tabName === 'visual-mapping') {
                ...
                loadESP32NodesForPlacement(); // Load ESP32 nodes for placement
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            loadAllFloors(); // Load floor plans for multi-room support
